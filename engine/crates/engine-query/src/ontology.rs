//! The node/edge ontology projection: additive,
...
//! deletable: read-and-infer only.

use engine_model::{NodeKind, Provenance, RelationKind};

/// The authority register a document type answers in:
...
/// rather than being silently coerced: an unparseable type is a data state,
/// never a fiction. `index` is intentionally NOT a register: `.vault/index`
/// feature-index documents are metanodes dropped at ingest, so they never reach this function; a stray one degrades to `unknown`.
...
/// The closed pipeline-derivation vocabulary. These labels say *what the relationship is in the framework* —
...
/// kind, the endpoint document types, and the edge provenance (assigned by
/// reading the `related:` provenance and the
...
/// exec record is generated by its parent plan (the record's id encodes
...
/// a per-type lifecycle status collapses to one
...
/// Project the per-type lifecycle status from the
...
mod tests {
    ...
    fn authority_class_covers_every_doc_type_and_unknown() {
        ...
        // ingest — it is not a register, so a stray one
    ...
    fn derivation_label_is_not_part_of_the_edge_stable_key() {
        ...
        // The WIDENED container-path detection
