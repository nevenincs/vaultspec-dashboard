...
//! broken edges are retained and surfaced, never dropped.
//!
...
pub struct Resolver {
    ...
    /// `Mutex<HashMap>` (not `RefCell`) so the parallel step fan-out
...
impl Resolver {
    ...
    /// exact step resolution logic with the parallel batch path via
    /// [`Resolver::resolve_one`] and [`Resolver::resolve_step_id`], so a single-doc resolve and a batch
    ...
    /// expensive step content scans across CPU cores (perf ADR D2;
    /// `graph-compute-is-cpu-gpu-is-render-and-search`).
    ///
    /// The win: `resolve_step_id` scans plan file contents once per DISTINCT
    /// step. That scan is a pure function of the step and the fixed tree, so it
    /// is embarrassingly parallel. This entry point:
    ///
    /// 1. Collects the DISTINCT step ids across all docs.
    /// 2. Primes the file-content cache by reading every plan file ONCE in
    ...
    ///    under the per-step scan).
    /// 3. Resolves the distinct steps in parallel (`par_iter`), each via the
    ///    SAME `resolve_step_id` logic as the sequential path, depositing
    ///    outcomes into the shared memo. Determinism holds because distinct
    ///    steps resolve independently.
    /// 4. Maps each document's mentions to `ResolvedMention` (memo lookups for
    ///    steps; wiki-link rules inline as before).
    ///
    ...
    pub fn resolve_batch(&self, per_doc: Vec<Vec<ExtractedMention>>) -> Vec<Vec<ResolvedMention>> {
        // 1. Distinct step ids across every document.
        let mut steps: Vec<String> = Vec::new();
        {
            let mut seen_step: std::collections::HashSet<&str> = std::collections::HashSet::new();
            for doc in &per_doc {
                for m in doc {
                    if let MentionKind::StepId(s) = &m.kind
                        && seen_step.insert(s.as_str())
                    {
                        steps.push(s.clone());
                    }
                }
        ...
        // 2. Prime the content cache once, in parallel — every plan file. After
        //    this the per-step scans never touch the filesystem, so the
        ...
        // 3. Resolve distinct steps in parallel. Each call deposits
        ...
        // 4. Map each document via memo lookups + wiki rules.
        per_doc
            .into_iter()
            .map(|mentions| {
                mentions
...
mod tests {
    ...
    fn fixture() -> tempfile::TempDir {
        ...
        std::fs::write(
            root.join("docs/2026-06-12-stale-adr.md"),
            "# outside vault\n",
        )
        .unwrap();
        std::fs::write(
    ...
    #[test]
    fn structural_states_assigned_for_step_ids_and_wiki_links() {
        let dir = fixture();
        let body = "Touches `src/lib.rs` and `old/moved.rs` and `gone/nothing.rs` as prose. \
                    Step `W01.P02.S03` and step `W09.P09.S99`. \
                    See [[2026-06-12-demo-adr]], [[2026-06-12-stale-adr]], \
                    and [[2026-06-12-missing-adr]]. \
                    Calls `insert()` and `vanished_function()` as prose.";
        ...
        assert_eq!(state_of("stale-adr").state, ResolutionState::Stale);
    ...
    fn resolver_reused_across_documents_resolves_consistently() {
        ...
        // (perf ADR D1); the step memo must return the SAME result for a
        ...
        // step id — the memo is exercised on the second.
        let doc_a = resolver.resolve(extract("Step `W01.P02.S03` and `W09.P09.S99`."));
        let doc_b = resolver.resolve(extract("Again `W01.P02.S03`, again `W09.P09.S99`."));
        assert_eq!(state_of(&doc_a, "W01.P02.S03"), ResolutionState::Resolved);
        assert_eq!(
            state_of(&doc_b, "W01.P02.S03"),
            ResolutionState::Resolved,
            "a memoized resolvable step stays resolved across documents"
        );
        assert_eq!(state_of(&doc_a, "W09.P09.S99"), ResolutionState::Broken);
        assert_eq!(state_of(&doc_b, "W09.P09.S99"), ResolutionState::Broken);
    }
    ...
    fn resolve_batch_is_byte_identical_to_sequential_resolve() {
        ...
        // edge states. The fixture exercises step ids and wiki-links across
        // multiple documents so distinct steps are deduplicated across docs and
        // the parallel fan-out is genuinely exercised.
        let dir = fixture();
        ...
        let docs: Vec<Vec<ExtractedMention>> = vec![
            extract(
                "Mentions source prose `src/lib.rs` and `gone/nothing.rs`. \
                 Step `W01.P02.S03` and step `W09.P09.S99`. \
                 See [[2026-06-12-demo-adr]], [[2026-06-12-stale-adr]], \
                 and [[2026-06-12-missing-adr]]. \
                 Calls `insert()` and `vanished_function()` as prose.",
            ),
            extract("Again step `W01.P02.S03`, plus [[2026-06-12-demo-adr]]."),
            extract(
