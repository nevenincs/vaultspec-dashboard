...
// ---------------------------------------------------------------------------
// Adversarial re-signing
// ---------------------------------------------------------------------------
//
// A refusal fixture is only worth its runtime if the repository it refuses is
// otherwise VALID — a tampered file that also breaks its own signature proves
// the signature check, not the refusal under test. These helpers re-sign the
// tampered role and every role that commits to it, so what reaches the verifier
...
pub fn resign<R: Role>(signed: R, pair: &Ed25519KeyPair) -> Vec<u8> {
...
/// Read and parse one signed role document.
...
// ---------------------------------------------------------------------------
// Named tampering verbs
// ---------------------------------------------------------------------------
//
// These wrap the re-signing primitives above so a consumer can express ONE
// tampering intent without naming a `tough` type. That matters beyond
// convenience: `vaultspec-product` deliberately keeps `tough` behind the
// distribution-authority boundary, and a test-only reason is not a good enough
// reason to breach it. Every verb leaves the repository signature-clean, so what
// the verifier refuses is the property under test and nothing else.

/// Freeze the timestamp role in the past, then re-sign it.
///
/// Only the expiry is wrong afterwards; the document still verifies against the
/// root, so the refusal is an expiry refusal rather than a signature one.
pub fn expire_timestamp_metadata(bundle: &Path, material: &SigningMaterial) {
    let path = bundle.join("metadata").join("timestamp.json");
    let mut timestamp: Signed<Timestamp> = read_signed(&path);
