...
impl AuthorityFile {
    ...
    /// (windows-private-file-authority D3/D4): the protected-state bit and the
    /// entry list drawn from ONE descriptor, for the safe [`private_policy`]
    /// validation layer. A matching entry list without the protected bit is not
    /// a private file; the snapshot carries both facts so the policy layer never
    /// joins two descriptor states.
