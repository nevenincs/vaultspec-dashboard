...
//!
//! THREE guards compose into [`authorize_command`], first-denial-wins:
...
//!
//! Review authority — whether the actor may decide a given proposal — is NOT a
//! guard here; it is owned end to end by [`super::policy::reviewer_eligibility`]
//! and enforced at the two live domain seams that back the served eligibility
//! (`super::approvals::review_decision_eligibility`, the apply preflight). No
//! production caller ever supplied this module an origin author to check.
//!
...
/// command carries targets), and the command's target documents. Borrowed; the engine
/// holds nothing.
///
...
/// delegation standing, then document scope. First denial wins; each denial is a
/// VALUE. A genuine registry read failure short-circuits as a redacted
/// [`SecurityFault`].
///
...
/// by their planes and still run downstream. Review authority (who may DECIDE a
/// proposal) is likewise not this module's concern — it is owned end to end by
/// [`super::policy::reviewer_eligibility`] at the two live domain seams.
