//! Discovery-file publication: the atomic
...
/// the bearer is minted before the heavy initial
...
/// Write the discovery file (rag's pattern). One process, one
/// port/token. `dir` is the publication target: the machine app home for a
/// SEATED serve, the workspace-local
...
/// to overwrite a foreign process's file.
...
/// clobber each other's discovery on every heartbeat tick. A missing or unparseable file is reclaimed (it is ours to heal).
...
/// `starting` from bind until the initial index
...
/// carrying our own pid is ours to retract. Missing
...
/// the previous complete payload or the new one.
...
mod tests {
    ...
    fn discovery_writes_are_atomic_under_concurrent_writers() {
        ...
        // The rename-based publish is what this
    ...
    fn heartbeat_refuses_to_overwrite_a_foreign_pid() {
        ...
        // heartbeat over: the owner check must refuse
