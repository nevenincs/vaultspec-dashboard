...
pub struct AppState {
    ...
    /// Graceful-shutdown latch: the bearer-gated `/shutdown` route (and the
    /// serve loop's ctrl-c/SIGTERM arm) raises it, and every long-lived
    /// response body observes it so the drain can actually finish. See
    /// [`ShutdownSignal`].
