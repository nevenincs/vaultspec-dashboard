//! Per-channel installation authority adapters.
//!
...
//! facts are what later mutation authority is gated on: install channel and
//! manager ownership come from a sealed adapter capability, never a
//! caller-selected `Channel` enum.
//!
...
impl InstallProvenanceAuthority {
    ...
    #[allow(
        dead_code,
        reason = "channel provenance is minted before the sealing transaction consumes it"
    )]
