...
- [ ] `W01.P01.S10` - Enforce installation transaction locking only for installer and copied-updater authority bind every guard to the canonical product lock identity require lock-first mutation ordering and owner-matching stale-state quarantine and forbid the gateway from acquiring or waiting on the lock; `engine/crates/vaultspec-product/src/locking.rs`.
