...
- [ ] `P01.S01` - Refactor the left aside into the ordered hosted-slot stack separated by soft 1px rules; `frontend/src/app/AppShell.tsx`.
- [ ] `P01.S02` - Preserve the collapse model and the single top-to-bottom focus order across the slots; `frontend/src/app/AppShell.tsx`.
- [ ] `P01.S03` - Apply attenuated-chrome tokens so the rail cedes attention to the stage; `frontend/src/app/AppShell.tsx`.
...
- [ ] `P02.S04` - Add a compact keyboard-reachable vault/code mode toggle to the browser region defaulting to vault; `frontend/src/app/left/`.
- [ ] `P02.S05` - Render the vault browser and the code-tree code mode behind the toggle; `frontend/src/app/left/`.
- [ ] `P02.S06` - Re-key the chosen mode per scope and wire it into the wholesale reset; `frontend/src/stores/view/`.
...
- [ ] `P03.S07` - Add an in-rail filter scoped to the active browser mode that narrows the already-fetched listing client-side; `frontend/src/app/left/`.
- [ ] `P03.S08` - Issue no wire request from the filter and clear it on scope swap; `frontend/src/app/left/`.
- [ ] `P03.S09` - Make the filter visibly distinct from the global right-rail search pillar; `frontend/src/app/left/`.
...
- [ ] `P04.S10` - Enforce that every rail interaction emits only scope-select, node-select, or view-affordance intent through stores; `frontend/src/app/left/`.
- [ ] `P04.S11` - Keep the inline git status badge read-only with no mutation affordance anywhere in the rail; `frontend/src/app/left/WorktreePicker.tsx`.
- [ ] `P04.S12` - Render the uniform four honest states across rail surfaces; `frontend/src/app/left/`.
- [ ] `P04.S13` - Establish the rail-wide keyboard contract, labelled landmark, and reduced-motion and keyboard-instant behaviour; `frontend/src/app/AppShell.tsx`.
...
- [ ] `P05.S14` - Test that the ordered rail stack renders with collapse and focus order; `frontend/src/app/`.
- [ ] `P05.S15` - Prove per-scope mode and filter reset with no cross-scope bleed; `frontend/src/stores/__adversarial__/`.
- [ ] `P05.S16` - Prove the read-only law has no fetch or mutation escape hatch in the rail; `frontend/src/app/left/`.
- [ ] `P05.S17` - Run the feature-scoped lint, test, and vault-check gates to green; `frontend/src/app/`.
