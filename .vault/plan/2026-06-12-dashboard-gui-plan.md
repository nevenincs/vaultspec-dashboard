...
- [x] `W02.P06.S26` - implement the node-scoped discover flow with visually quarantined, session-pinned-only semantic candidates per G3.c; `frontend/src/app/stage/Discover.tsx`.
- [x] `W02.P06.S27` - implement node pinning, layout-fixed and always-labelled, with client-side persistence per G5.d; `frontend/src/stores/view/pins.ts`.
...
- [x] `W02.P07.S31` - implement named lenses saved client-side and exposed to the command palette per G3.f and G5.d; `frontend/src/stores/view/lenses.ts`.
...
- [x] `W03.P11.S45` - implement the rag-down text-match fallback with the explicit semantic-search-offline state per G8.a; `frontend/src/app/right/searchFallback.ts`.
...
- [x] `W03.P12.S46` - implement the degradation matrix states with a debug switch making every state reachable, and tests per G8.a; `frontend/src/app/degradation`.
