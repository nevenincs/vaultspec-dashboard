...
- [ ] `W01.P02.S05` - implement the contract-shaped scene graph model with node kind, lifecycle, degree-by-tier and edge relation, tier, confidence, state per G3.a and the contract identity guarantees; `frontend/src/scene/graphModel.ts`.
...
- [ ] `W01.P03.S09` - mount the PixiJS application behind the SceneController lifecycle of mount, resize, destroy per G6.b; `frontend/src/scene/field/pixiField.ts`.
