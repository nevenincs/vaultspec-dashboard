...
1. **Backend signals stream**: Always-on, hidden-tab pause, no cursor (current state only)
1. **Authoring lifecycle stream**: `last_seq` cursor, recovery snapshot, dedicated state machine
...
1. TanStack cache eviction (`gcTime: 30_000` on streams) — cursor lost if unobserved
1. Navigate away and back — fresh stream starts from beginning
1. Ring buffer eviction on engine (server-side gap) — client must detect and re-keyframe

**Silent divergence risks**:

1. Slow consumer lag — handled by `gap` events with `lagged` count
1. Network partition during write — monotonic seq ensures detection on reconnect
1. Scope switch without re-subscription — cross-scope contamination prevented by scope parameter
...
- `frontend/src/stores/server/authoring/adapters.ts:38-55` — AUTHORING_STREAM\_\* timing constants
