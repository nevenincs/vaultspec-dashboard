...
modified: '2026-09-19'
body_schema: 'body-v1'
body_hash: 'sha256:7e9dfa0192c64541cb7f133360501b4dff237f6db446ccf8976135ad2bf3cd0f'
related:
...
1. Icons only, no wire change (client-side type icons; defer git/ignore display) — half the legibility, avoids engine work now; the wire half lands later.
1. Author file-type marks in-family on Phosphor's grid — no third family, but monochrome marks reproduce neither VS Code's recognition speed nor the owner's stated expectation, and hand-authoring dozens of marks is sustained cost.
...
- `material-icon-theme` (npm package) — the adopted icon-theme library; vendored at `frontend/package.json:44` and consumed as a tree-shaken subset in `frontend/src/app/left/fileIcons.generated.ts`, regenerated by `frontend/dev/tooling/generate-file-icons.mjs`.
- `https://marketplace.visualstudio.com/items?itemName=PKief.material-icon-theme` and `https://github.com/material-extensions/vscode-material-icon-theme` — marketplace listing and source repository for the extension id, install count, and MIT license.
- `https://code.visualstudio.com/api/references/theme-color` — VS Code theme-color reference, Git colors section, documenting the `gitDecoration.modifiedResourceForeground` / `untrackedResourceForeground` / `addedResourceForeground` / `deletedResourceForeground` / `renamedResourceForeground` / `conflictingResourceForeground` / `ignoredResourceForeground` tokens.
- `https://jeremyshanks.com/top-10-best-vs-code-icon-themes` — third-party installs-ranking survey supporting the "most-installed icon theme" claim.
- `https://nextron-systems.com` (2025-11-28 impersonator advisory) — supply-chain caveat on a marketplace impersonator of the genuine theme.
- `frontend/src/stores/server/engine/graphTypes.ts:218` — the served `FileTreeEntry` shape (`path`, `kind`, `has_children`, `node_id`, plus `git_status?`/`ignored?`).
- `engine/crates/ingest-git/src/file_tree.rs` — the engine's worktree tree walk and ignore-provenance handling, using gix's own exclude machinery (`gix-ignore`) with `.vaultspecragignore` riding the same matcher as a second pattern set.
- `frontend/src/stores/server/queries/gitchanges.ts` — the per-scope changed-file status reads (`ChangedFile`, `GitChangesSummary`) consumed by the Changes fold.
- `engine/crates/vaultspec-session/src/settings_schema.rs` — the single settings-schema registry a toggleable icon-set setting would be declared in.
