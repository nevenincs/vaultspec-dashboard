...
<!-- FRONTMATTER RULES:
     ...
     The S06 and 2026-07-03-search-providers-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Extract the one shared literal matcher utility with the explicit bands (strong-literal 0.70 to 0.95 for exact or prefix, weak-literal 0.20 to 0.50 for substring), token matching over stem, path, title, and tags, with unit vectors, replacing the two near-duplicate scanners and ## Scope

- `frontend/src/stores/server/literalMatch.ts` placeholders below are machine-filled
