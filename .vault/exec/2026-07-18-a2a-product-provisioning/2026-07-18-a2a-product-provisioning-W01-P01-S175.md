...
The internal terminal-settlement callback remains outside the machine-bearer prefix set by design and continues to authenticate through its required `AttachControlAuth` extractor. It is now present in the canonical `CONTRACT_ROUTES` inventory and in an explicit `ATTACH_CONTROL_ROUTES` authentication class. The route-inventory guard therefore detects an unclassified router addition, the machine-bearer sweep covers only its declared class, and a direct route proof rejects both a tokenless request and the dashboard machine bearer. On Windows, each of the six settlement scenarios stops at the exact typed credential bootstrap gate before any credential, lease, callback, receipt, settlement, expiry, or restart-reconciliation effect; no credential is fabricated to bypass the accepted gate.

S175 remains open. The accepted D7 descriptor is not complete: it does not yet bind verified distribution identity, sealed install provenance, or the intended receipt; its phase advance still rewrites in place rather than through an interruption-safe format; pending cleanup/disarm authority is incomplete; and a crash after receipt settlement but before descriptor retirement cannot be resolved safely without that binding. The accepted Windows private-file-authority decision now has a purpose-separated D9 safe substrate and real local-NTFS authority tests, but product credential/bootstrap policy and distribution rollback-state consumers have not migrated to it. Windows therefore intentionally remains typed unavailable before descriptor or secret creation and before credential disclosure. The plan checkbox must remain open until D7 is implemented and reviewed, the exact three-principal Windows policy and both consumer migrations pass independent review and supported-target NTFS evidence, real Unix interruption/adversarial evidence passes, and a hard execution bound exists that does not falsely report cancellation while filesystem mutation continues.
