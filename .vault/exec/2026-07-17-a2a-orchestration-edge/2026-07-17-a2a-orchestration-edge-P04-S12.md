...
Approved design decisions recorded: single-document batch (latest-document-wins, visible reset), source_revision = the open document's blob hash (the D4 staleness fence is a known-unenforced P05 partial), comments-only submit via an empty prompt plus the batch id. No skips introduced. S14 (worker feedback ingest) follows.
