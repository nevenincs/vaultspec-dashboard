...
P11.1 verification (tenth entry): **W03.P11 is CLOSED** at `45b5eae`. Reviewer-verified in code: N2 - historical diffs are result-local (`"clock": "result-local"` on the payload; the shared atomic is mutated only by the commit path); N1 - subscribe-first splice order with the audit finding cited in the comment; N6 - `API_PREFIXES` JSON-404 in the SPA fallback; N7 - THE shared `api_error` helper attaches the tiers block to every error; N3/N4 - `commit_graph(fresh)` is the single diff-swap-bump-broadcast path; N8 - `paginate` wired on the query listings; N5 - rag whitelist now carries lifecycle (server start/stop/status) + reindex + watcher-status, with watcher-reconfigure DEFERRED with recorded rationale (an unvalidated argument channel is worse than a missing verb - accepted as a flagged bound, revisit when rag's reconfigure arguments can be validated); N12's consumer half (dual-format heartbeat parsing with staleness truthfulness) verified live in P10.1. Dogfood fixes verified: DF-2 launch-dir resolution, DF-4 hardening - `busy_timeout(10s)` on the store plus a panic hook writing `crash.log` and stderr (silent death eliminated). RESIDUAL (routed to P12): the watcher thread has no JoinHandle monitoring - a watcher-thread panic is now logged but leaves a zombie service (heartbeat alive, rebuilds stopped, /status still reporting a resident watcher). P12 must add watcher-death surfacing in `/status` and a concurrent CLI-index-during-serve case in the e2e suite (which also empirically retires the DF-4 reproduction question).

Dogfood dispositions addendum (with team-lead's exact repros): DF-2 CLOSED at `45b5eae` (serve resolves the launch directory to its worktree like every verb; live-verified from a subdirectory). DF-4 CLOSED - the executor's repro now passes (serve SURVIVES a parallel one-shot index and answers /status after; busy_timeout plus crash-log hook); the P12 e2e concurrent case remains as the regression guard, and the zombie-watcher residual stands. DF-5 CLOSED as route confusion: `/graph` was never a route (`/graph/query` is); pre-N6 the SPA fallback answered it, post-N6 it is a JSON 404 - and no warming window exists at all, since the cold initial index completes before the listener binds. Rider kept from the repro: document the scope token format in the `/map` response (P12 nicety). DF-6 OPEN, contract-level: SPA token bootstrap is unspecified (browser cannot read service.json; everything but /health is bearer-gated). Ruling proposed to the GUI side: engine injects the token into served index.html (meta tag), dev handled by proxy-side Authorization injection from service.json, Host-header validation as a rebinding rider; loopback-bypass and cookie options rejected with rationale. Contract amendment lands on agreement; implementation routes to P12. Upstream: the rag service.json schema filing is live as `wgergely/vaultspec-rag#190` (D5.3 discharged).

DF-6 RESOLVED at contract level (agreed both sides): token injection into served index.html, with the GUI side's two riders folded into the amendment - fetch-based SSE consumption named as required (native EventSource cannot carry the bearer), and 401 defined as the canonical stale-token re-bootstrap signal (a designed degraded state); meta-tag absence legal for dev-proxy mode; Host validation rider. Contract reference section 1 amended. Implementation routes to P12.

Two new lows from team-lead's live probe, routed to P12 with rulings: (L1) `GET /map` over HTTP returns a bare object while the contract section 2 envelope (`data` + `tiers` + cursor) is the canonical wire shape - RULED: every HTTP route carries the section-2 envelope; the CLI's ok/command/status envelope is the CLI's own vocabulary (D6.2), the HTTP side conforms to contract section 2; audit all routes for envelope consistency, with tests. (L2) Scope tokens are wire-inconsistent: `/map` worktree paths carry the Windows verbatim prefix (`//?/Y:/...`) while edge scopes are clean (`Y:/...`) - since scope is the stateless request parameter everywhere, inconsistent token forms cause silent scope mismatches. RULED: one canonical form (no verbatim prefix, forward slashes) on every wire surface, and the `/map` response documents the scope token format (absorbing the earlier DF-5 rider).

P10.1 verification (ninth entry): **W03.P10 is CLOSED** at `321129b`. Reviewer-verified in code and suites: failure envelopes carry the tiers block (G1); `--granularity document|feature` with `meta_edges` emitted (G4); typed usage-class exit 2 with machine-readable error kinds and `--scope` validated against discover-plus-enumerate (live: arbitrary directories rejected); event sourcing lifted to `engine_query::events::commit_rows` and consumed by BOTH front doors (G7 - this also pre-fixes the serve `/events` source); status carries git head/dirty (G5); index speaks the core vocabulary with the recorded created/removed-inapplicable interpretation (G2); first-class `corpus_views` (G3); per-verb envelope integration tests plus typed-exit and prefix-leak cases (G8); riders folded. Bonus beyond the set: rag discovery now searches project-then-home candidates and parses both heartbeat formats with stale-heartbeat truthfulness - this closes the N12 half of the P11.1 set early (live: semantic available true against the running rag service). N12 remains listed under P11.1 only for the upstream schema filing, which team-lead carries.

## W03P12-801 | low | P12 closure WITHHELD on exactly the three crossed items (P12.1) - all else verified excellent

Eleventh entry: final phase boundary, W03.P12 at `f190083` (plan 56/56 pending this review). Reviewer-verified: the e2e workspace member with the first true multi-corpus-view facet exercise (one node, two views, divergence surfaced precisely on the branch-edited document); CLI/serve parity on identical id sets in identical order with byte-stable repeats; the concurrent CLI-index-during-serve case (serve survives, watcher alive - DF-4 empirically retired); degradation-path coverage; the env-gated REAL-repository leg run live once with clean teardown; bench baseline recorded with the honest warm-equals-cold explanation (resolution is deliberately uncached live signal) and the next lever named as future work; engine CI on both platforms; carry 104 closed (per-pass memoized reads, bounded root-gitignore honoring with globs rationale-excluded); watcher-death surfacing exactly as prescribed ("/status" states a dead watcher with reason, never papered over); 27 suites green at HEAD. The autocrlf operator footgun note in the S54 record is appreciated signal.
