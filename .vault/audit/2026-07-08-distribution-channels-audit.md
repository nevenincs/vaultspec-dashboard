...
date: '2026-07-08'
modified: '2026-07-08'
body_hash: 'sha256:03c568ad3c95ccbb50ccaeaf9241bb64a14243f23a1df10a20cf150dfdbf8504'
related:
...
### post-review live-loop closure (2026-07-08)

Three sibling-collision test adoptions landed after the reviewed range as the in-flight authorization floor kept moving: the vertical slices and langgraph fixture thread the server-authoritative scope token (`fcb01a4cfa`, `b4c4ac24b7`) and the frontend live tests reuse the served token, all suites green on CI. The first live release through the full chain (v0.1.1) surfaced and fixed the dist/release-please duplicate-create collision plus the workflows-directory fragment parsing (`e8f054ff1a`, recorded as a release-automation ADR amendment). The loop then PROVED itself: tag minted by the release PR merge, five targets uploaded, the scoop-bump bot committed `f561f32b43`, and an end-user `scoop install vaultspec` delivered 0.1.1 on this machine. The winget submission passed Microsoft's validation pipeline (`Azure-Pipeline-Passed`, `Validation-Completed`) with no Defender friction and awaits moderator merge.
