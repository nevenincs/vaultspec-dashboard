---
generated: true
tags:
  - '#index'
  - '#graph-lineage-dag'
date: '2026-08-01'
modified: '2026-08-01'
body_schema: 'body-v1'
body_hash: 'sha256:b7296ae73c97a415ad666b91dd6a23f188e8a2f7d20f05e60acae52b4c8de0bd'
related:
  - '[[2026-06-16-graph-lineage-dag-adr]]'
  - '[[2026-06-16-graph-lineage-dag-audit]]'
  - '[[2026-06-16-graph-lineage-dag-research]]'
---

# `graph-lineage-dag` feature index

Auto-generated index of all documents tagged with `#graph-lineage-dag`.

## Documents

### adr

- `2026-06-16-graph-lineage-dag-adr` - `graph-lineage-dag` adr: `rebuilding the lineage mode as a Sugiyama-layered DAG and closing the engine derivation-labeling hole that starves it` | (**status:** `accepted`)

### audit

- `2026-06-16-graph-lineage-dag-audit` - `graph-lineage-dag` audit: `aggregate-LOD super-node render deferral`

### research

- `2026-06-16-graph-lineage-dag-research` - Lineage DAG rebuild research
