---
generated: true
tags:
  - '#index'
  - '#graph-simulation-stability'
date: '2026-06-29'
modified: '2026-06-29'
related:
  - '[[2026-06-29-graph-simulation-stability-S01]]'
  - '[[2026-06-29-graph-simulation-stability-S02]]'
  - '[[2026-06-29-graph-simulation-stability-S03]]'
  - '[[2026-06-29-graph-simulation-stability-S04]]'
  - '[[2026-06-29-graph-simulation-stability-adr]]'
  - '[[2026-06-29-graph-simulation-stability-plan]]'
  - '[[2026-06-29-graph-simulation-stability-research]]'
---

# `graph-simulation-stability` feature index

Auto-generated index of all documents tagged with `#graph-simulation-stability`.

## Documents

### adr

- `2026-06-29-graph-simulation-stability-adr` - `graph-simulation-stability` adr: `the settled layout is authoritative: additive set-data pins survivors and relaxes only new nodes` | (**status:** `accepted`)

### exec

- `2026-06-29-graph-simulation-stability-S01` - Unify the warm-start dispatch so every warm path pins survivors and relaxes only new nodes via prewarmReflow
- `2026-06-29-graph-simulation-stability-S02` - Add a solver test asserting an additive setData pins survivors (no movement) and a no-new-node setData runs zero ticks
- `2026-06-29-graph-simulation-stability-S03` - Run the full frontend gate (just dev lint frontend) and the test suite to exit 0
- `2026-06-29-graph-simulation-stability-S04` - Live-verify: expanding a node on a large graph holds existing positions

### plan

- `2026-06-29-graph-simulation-stability-plan` - `graph-simulation-stability` plan

### research

- `2026-06-29-graph-simulation-stability-research` - `graph-simulation-stability` research: `graph stays static unless dragged: decoupling selection and additive set-data from the force solver`
