...
- D1 — Wire: `FeatureRosterEntry` grows `type_counts` (doc-type → count over the FULL corpus), `plan_state?` (`planned` | `in-progress` | `finished`, absent when the feature has no plan; engine rollup: finished iff all plans finished, in-progress iff any in progress, else planned), and `adr_dates?` (`{first, last}` ISO dates over the feature's ADRs; absent when no ADR). Additive, tolerant-adapter absorbed; a contract event reviewed once here.
...
- D3 — Status mark occupies the feature row's ICON slot (one icon per row): finished = the acceptance/check mark, in-progress = the progress mark, planned = the plan mark, no-plan = the existing feature glyph. Marks come from the already-sanctioned Phosphor status marks; served token → presentation mapping only.
