...
**Amendment (graph-representation, 2026-06-14): a representation-mode selector and a lens selector join the family.** The salience and representation trio (`graph-node-salience-adr`, `graph-node-semantics-adr`, `graph-representation-adr`) was authored after this ADR and introduces two new on-stage control groups this family must home, named here so an executor does not invent them:

- **Representation-mode selector.** A new control group selecting the active representation mode (connectivity, lineage, semantic). It is EXPLICITLY DISTINCT from, and RECONCILED WITH, the existing force/circular toggle in `AlgorithmPanel.tsx`: representation mode changes WHICH CPU-worker layout runs and what data it consumes (connectivity = ForceAtlas2, lineage = the derivation-axis DAG, semantic = the UMAP projection over embeddings), while force/circular only tunes the force SOLVER. The reconciliation is that **force/circular becomes a sub-option of the connectivity mode** — the FA2/circular toggle and its sliders apply only when the representation mode is connectivity, and dim (like the FA2 sliders in circular mode already do) in lineage/semantic where a force solver does not run. The selector emits a `set-representation-mode` scene command through `SceneController.command(...)` (scene-only render concern, no wire, no engine coordinates) and reads its initial truth from `getRepresentationState()`, exactly as the layout panel reads `getLayoutState()`. The semantic mode is v1-gated; when its promotion gate holds it, the scene downgrades to connectivity and echoes the applied mode, which the selector reflects honestly (a held option reads as unavailable, never as silently ignored).
- **Lens selector.** A new control group selecting the active SALIENCE lens (status, design) — the viewer-intent parameter that, via DOI, drives both the salience importance field and the served node set. Unlike the representation-mode selector (a scene command), the lens selector emits lens intent into the stores VIEW STORE, because switching the lens is a wire RE-QUERY (the lens folds into the graph slice cache key) — the stores layer is the sole wire client, and the lens is a query parameter, not a render knob. This is the lens-selector control group the salience ADR flagged as a required amendment; it is distinct from the named-filter-set lenses (the palette's saved filters) and from the tier dial.

Both new controls inherit the family's base tokens, motion grammar, density, icon sources (Lucide structural, Phosphor-grid domain marks), and the four canonical states. The composition rule (`graph-representation-adr`) governs their interaction: a lens switch re-queries then the active mode re-lays-out; a mode switch re-lays-out the current set with no re-query — the stores layer sequences the two so they never contend, and every lens is viewable in every mode.
