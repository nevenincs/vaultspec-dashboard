...
- D3 â€” Displayed state stays served: `FileTreeEntry` grows `git_status?` (token vocabulary: `modified` / `added` / `deleted` / `renamed` / `untracked` / `conflicted`, absent = clean) and `ignored?` (absent = not ignored, `git` | `rag` = which ignore file matched). The level body additionally carries `status_truncated: bool` so a status join capped by the snapshot bound is stated, never a partial join reading as clean (amended in place when execution surfaced the case). Counts or rollups, if ever shown, are engine-computed. The frontend maps tokens to presentation only.
- D4 â€” Ignore truth comes from gix: the walk adopts gix's own exclude machinery (`gix-ignore`, already in the dependency tree) evaluating `.gitignore` semantics properly, with `.vaultspecragignore` patterns fed through the same matcher and reported distinctly. Ignored entries are SERVED AND SHOWN (dimmed), not hidden; this supersedes the walk's hide behavior entirely: the directory-name pattern collection, the dot-directory hide, and the hard-coded build-tree set are all retired (only `.git` itself stays withheld as storage, not content). The ripgrep `ignore` crate is rejected â€” it would be a second gitignore implementation beside gix.
