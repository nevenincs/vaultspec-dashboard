...
<!-- FRONTMATTER RULES:
     ...
     Replace command-palette-actions with a kebab-case feature tag, e.g. #foo-bar.
...
# `command-palette-actions` adr: `UI action taxonomy, shortcuts, and backend verb feed` | (**status:** `{proposed|accepted|rejected|deprecated}`)
