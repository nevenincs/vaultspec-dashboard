...
export const GRAPH_CONTROL_SCHEMA = [
  ...
  },
  {
    ...
    exposure: [],
    description:
      "Fixed pixel margin reserved on every edge when framing (fit-to-view / fit-to-nodes / autoframe), UI-scaled. The framed graph fits the canvas minus 2×this per axis, so node bodies never touch the rim. A true px gap (zoom-independent), replacing the old fractional fitPaddingFactor whose apparent margin shrank as the graph span grew.",
  },
  {
    id: "minimapInset",
    label: "Minimap inset",
