...
// are the multi-seed difficulty-swept calibration baseline produced by
// `calibrate.ts` (`calibrateAll`), run once by hand and committed here. Each floor is
...
// deliberate, reviewed act: run `calibrate.ts`, read `formatCalibration`, update the
// literals below, and bump METRIC_VERSION (in `scorecard.ts`) if any metric
// DEFINITION changed (the thresholds are matched to a metric version). The
// `calibrate.test.ts` smoke test asserts these committed constants reproduce from the
// sweep (within the margin) and that the sweep is deterministic.
//
...
//   force/sbm        stress 0.869->0.82  neighborhoodPreservation 0.193->0.14
//                    nodeResolution 0.130->0.08  edgeLengthUniformity 0.543->0.49
//                    crossings 0.934->0.88  crossingAngle 0.745->0.69
