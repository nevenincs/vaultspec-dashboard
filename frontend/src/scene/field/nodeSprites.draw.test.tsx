...
/** A glyph provider that never touches the GPU: the body is a Graphics circle
 *  now (not a textured sprite), so textureFor is only the fallback seam. */
const stubGlyphs: GlyphTextureProvider = {
...
/** Count Graphics children of the layer container (bodies + rings + stamps). */
...
describe("NodeSpriteLayer — category-coloured body circle", () => {
  it("draws one filled-circle body per node, sized by salience", () => {
    ...
    const layer = new NodeSpriteLayer(world, stubGlyphs);
    layer.sync(
      model([
