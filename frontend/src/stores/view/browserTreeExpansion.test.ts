...
describe("browser tree expansion store", () => {
  ...
  it("derives code-tree row state and chrome from tree inputs", () => {
    ...
    expect(
      ...
      rowClassName:
        "flex h-[1.875rem] w-full select-text items-center gap-fg-1 truncate rounded-fg-xs pr-fg-1 text-meta text-left transition-colors duration-ui-fast ease-settle focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-focus bg-accent-subtle font-medium text-ink",
