...
import { useDashboardTextFilterIntent } from "../server/dashboardTextFilterIntent";
 ...
 */
export function useDashboardTextFilterDraft(
  scope: string | null,
): DashboardTextFilterDraft {
  const { canonicalText, writeTextFilter } = useDashboardTextFilterIntent(scope);
  ...
  }, [canonicalText, debouncedSetTextFilter, scope]);

  const setValue = useCallback(
    (next: string) => {
      setLocalValue(next);
      debouncedSetTextFilter(next);
    },
