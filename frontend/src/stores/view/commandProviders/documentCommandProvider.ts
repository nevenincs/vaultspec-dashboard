// Document-scoped command provider. Wraps the pure `buildDocumentCommands` core
// (copy-link), reading the active document's stem from the CommandContext, and
// self-registers. The command enrolls ONLY when a document is open, so it is never a
// dead palette entry with no target.
