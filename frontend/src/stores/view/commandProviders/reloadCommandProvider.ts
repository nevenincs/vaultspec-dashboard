// Reload/refresh command provider. The "reload commands" the brief called out as
// missing: a client-side refresh
...
export function reloadCommandProvider(): readonly unknown[] {
  // Compose the SHARED Refresh builder: the palette, the Mod+Shift+R chord, and the
  // context-menu global tail all run the same descriptor, so the verb cannot drift.
  // The palette groups by `family`, not the descriptor's section.
