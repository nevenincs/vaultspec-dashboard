...
describe("ComposerExpertSelection role labels", () => {
  ...
  it("treats a prototype-colliding served role as absent until its exact override is enabled", () => {
    const emittedOverrides: Readonly<Record<string, ProviderCatalogSelection>>[] = [];
    const runtime = createTestLocalizationRuntime();
    render(
      <I18nextProvider i18n={runtime}>
        <ComposerExpertSelection
          requiredRoles={["constructor"]}
          requiredRoleLabels={{}}
          providers={[provider]}
          selection={selection}
          overrides={{}}
          fallbacks={[]}
          onChangeOverrides={(next) => emittedOverrides.push(next)}
          onChangeFallbacks={() => {}}
          locked={false}
        />
      </I18nextProvider>,
    ...
    expect(screen.getByText(/Agent 1/)).toBeTruthy();
    const roleSwitch = screen.getByRole("switch", { name: /Agent 1/ });
    expect(roleSwitch.getAttribute("aria-checked")).toBe("false");

    fireEvent.click(roleSwitch);

    expect(emittedOverrides).toHaveLength(1);
    expect(Object.keys(emittedOverrides[0]!)).toEqual(["constructor"]);
    expect(emittedOverrides[0]!["constructor"]).toEqual(selection);
  });
});
