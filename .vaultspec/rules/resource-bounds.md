...
- **Dev artifact stores are scoped and reclaimable.** Worktree builds share one `CARGO_TARGET_DIR`; a merged agent worktree is torn down (`git worktree remove --force`); rag model downloads are scoped with `HF_HOME`; `just clean` reclaims the lot. No unbounded per-worktree global sprawl.
